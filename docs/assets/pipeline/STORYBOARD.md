...
- Header: identifies the general-purpose downloader and states the quality gate: resolve, recover within a bound, then publish after media and output checks. UniFi compatibility is an optional output profile, not the default.
- 01 Input URL: accepts a single-video URL; playlist input is rejected by the single-video contract.
- 02 Resolve metadata + formats: yt-dlp extracts metadata and selects the highest-quality dedicated video and audio formats, or one progressive format when no complete pair is available.
...
- 04 Download streams: the dedicated pair is downloaded separately, while a progressive fallback is downloaded once as one combined input; partial files are cleaned up on failure or cancellation.
- 05 Remux or transcode: generic output preserves compatible source streams in Matroska when possible; the optional UniFi profile targets H.264/AAC MP4 at the supported 24/25/30 FPS target.
- 06 Publish without overwrite: the publisher copies the owned source to a private temporary file in the selected destination, verifies its positive exact length, commits with a same-directory non-overwriting rename, and re-verifies the final output. Existing destinations produce a safe conflict; no suffix or hard-link/whole-operation atomicity claim is made.
