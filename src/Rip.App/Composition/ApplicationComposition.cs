...
public static class ToolManifestLoader
{
    ...
    public const string ManifestFileName = "rip.tools.json";
    public const string ManifestPathEnvironmentVariable = "RIP_TOOL_MANIFEST";

    public static string DefaultManifestPath => File.Exists(Path.Combine(AppContext.BaseDirectory, ManifestFileName))
        ? Path.Combine(AppContext.BaseDirectory, ManifestFileName)
        : Setup.ToolBootstrapper.ManifestPath;
...
public static class ApplicationComposition
{
    public static ComposedApplication Create(
        ...
        Updates.IAppUpdateService? updateService = null)
    {
        ...
        try
        {
            ...
            var viewModel = new DownloadViewModel(updateService);
            var downloads = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "Downloads");
            if (Directory.Exists(downloads)) viewModel.OutputFolder = downloads;
            foreach (var tool in Enum.GetValues<ToolKey>())
            {
