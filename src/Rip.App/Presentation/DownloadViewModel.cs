...
public sealed record VideoQualityOption(string Label, int? MaximumHeight)
{
...
public sealed class DownloadViewModel : INotifyPropertyChanged
{
    ...
    private VideoQualityOption selectedQuality = new("Highest available", null);
    ...
    public DownloadViewModel(Updates.IAppUpdateService? updateService = null)
    {
        canOpenFolder = false;
        Updates = new Updates.AppUpdateViewModel(updateService ?? new Updates.DevelopmentUpdateService(), () => IsBusy);
        Updates.PropertyChanged += (_, _) => Notify(nameof(CanStart), nameof(CanRetry));
    }

    public Updates.AppUpdateViewModel Updates { get; }

    public string VideoUrl { get => videoUrl; set { if (Set(ref videoUrl, value ?? string.Empty)) NotifyFormChanged(); } }
    public VideoQualityOption SelectedQuality { get => selectedQuality; set { if (value is not null && Set(ref selectedQuality, value)) NotifyFormChanged(); } }
    public IReadOnlyList<VideoQualityOption> VideoQualities { get; } =
    [new("Highest available", null), new("4K · 2160p", 2160), new("1440p", 1440), new("Full HD · 1080p", 1080), new("HD · 720p", 720), new("480p", 480), new("360p", 360)];
    ...
    public bool IsBusy { get => isBusy; private set { if (Set(ref isBusy, value)) { Notify(nameof(CanStart), nameof(CanCancel)); Updates.RefreshAvailability(); } } }
    ...
    public string ErrorMessage { get => errorMessage; private set { if (Set(ref errorMessage, value)) Notify(nameof(HasError)); } }
    public string MetadataSummary { get => metadataSummary; private set { if (Set(ref metadataSummary, value)) Notify(nameof(HasMetadata)); } }
    public string CompletionText { get => completionText; private set { if (Set(ref completionText, value)) Notify(nameof(HasCompletion)); } }
    public bool HasError => !string.IsNullOrWhiteSpace(ErrorMessage);
    public bool HasMetadata => !string.IsNullOrWhiteSpace(MetadataSummary);
    public bool HasCompletion => !string.IsNullOrWhiteSpace(CompletionText);
    ...
    public bool CanStart => !IsBusy && !Updates.IsInstalling && !IsTerminal && !HasMissingCapability && IsFormValid;
    public bool CanRetry => !IsBusy && !Updates.IsInstalling && ScreenState == ScreenState.Failed &&
    ...
    public string StartHelpText
    {
        get
        {
            ...
            {
                return "Paste a video link to get started.";
            }
            ...
            return "Ready to download. Your original files will be kept.";
        }
    }
        ...
        try
        {
            ...
            var output = SelectedOperation == DownloadOperation.Metadata
                ...
                : new OutputOptions(
                    ...
                    FrameRateTarget,
                    compatibility,
                    SelectedOperation == DownloadOperation.Video ? SelectedQuality.MaximumHeight : null);
