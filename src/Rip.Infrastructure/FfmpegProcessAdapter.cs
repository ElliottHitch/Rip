...
public sealed class FfmpegProcessAdapter
{
    ...
    private static readonly TimeSpan DefaultTimeout = TimeSpan.FromHours(12);
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // Provider codec labels do not establish pixel format, AAC profile or bitrate.
            // Enforce the complete Connect output profile instead of trusting those labels.
            ...
            if ((plan.Request.Output.UnifiCompatible || plan.Request.Output.Container == OutputContainer.UnifiMp4) &&
                lengthBytes > 5L * 1024 * 1024 * 1024)
                return Failure<MediaProcessingResult>(SafeDownloadError.Create(
                    DownloadErrorCode.TooLarge, DownloadStage.Processing,
                    "This video exceeds UniFi Connect's 5 GB limit. Choose a lower quality or turn compatibility off.",
                    RetryAction.UserActionRequired, new RedactedDiagnosticToken("diag-connect-size-limit")));

            if (!await MediaOutputVerifier.VerifyAsync(executor, outputPath, plan, cancellationToken).ConfigureAwait(false))
                return Failure<MediaProcessingResult>(SafeInfrastructureErrors.OutputVerificationFailed());
        ...
        out FrameRateDecision frameRate)
    {
        ...
        try
        {
            ...
            frameRate = FrameRatePolicy.Decide(
                ...
                plan.Request.Output.UnifiCompatible || plan.Request.Output.Container == OutputContainer.UnifiMp4);
        }
    ...
    private static class FfmpegCommandBuilder
    {
            ...
            string outputPath)
        {
            ...
            else
            {
                ...
                {
                    arguments.AddRange(["-c:a", "aac", "-profile:a", "aac_low", "-b:a", "192000", "-ac", "2"]);
                }
            }
