...
{
    private const int MaximumFormats = 2048;
        ...
        CancellationToken cancellationToken)
    {
        ...
        var maximumHeight = request.Output.MaximumVideoHeight;
        if (maximumHeight is <= 0)
            return Failure<MediaPlan>(SafeInfrastructureErrors.UnsupportedProviderFormat(DownloadStage.Resolving));
        if (request.Output.UnifiCompatible || request.Output.Container == OutputContainer.UnifiMp4)
            maximumHeight = Math.Min(maximumHeight ?? 2160, 2160);
        var formats = parsed.Value!.Formats
            .Where(format => selection != FormatSelection.Video || !format.HasVideo ||
                maximumHeight is null || format.Height is > 0 && format.Height <= maximumHeight)
            .ToArray();
    ...
    private static ProviderResult<MediaPlan> BuildVideoPlan(DownloadRequest request, FormatDescriptor video, FormatDescriptor audio)
    {
        ...
        return new ProviderResult<MediaPlan>(new MediaPlan(
            ...
            video.LengthBytes,
            audio.LengthBytes,
            VideoHeight: video.Height is { } height ? (int)height : null), null);
    }
    ...
    private static ProviderResult<MediaPlan> BuildProgressivePlan(DownloadRequest request, FormatDescriptor progressive)
    {
        ...
        return new ProviderResult<MediaPlan>(new MediaPlan(
            ...
            null,
            IsProgressive: true,
            VideoHeight: progressive.Height is { } height ? (int)height : null), null);
    }
    ...
    private static class ProviderJsonParser
    {
        ...
        public static ParseResult<ProviderDescription> Parse(string json)
        {
            try
            {
                ...
                if (formatsElement.GetArrayLength() > MaximumFormats) return ParseResult<ProviderDescription>.Invalid;
                foreach (var element in formatsElement.EnumerateArray())
                {
                    // YouTube includes storyboard/image entries and DRM-only formats alongside media.
                    // They are not downloadable audio/video and must not invalidate the whole video.
                    if (element.ValueKind == JsonValueKind.Object &&
                        (RequiredText(element, "ext") == "mhtml" ||
                         RequiredText(element, "vcodec") is null || RequiredText(element, "acodec") is null ||
                         RequiredText(element, "vcodec") == "none" && RequiredText(element, "acodec") == "none" ||
                         element.TryGetProperty("has_drm", out var drm) && drm.ValueKind == JsonValueKind.True)) continue;
                    if (!TryFormat(element, out var format)) return ParseResult<ProviderDescription>.Invalid;
                    formats.Add(format!);
                }
        ...
        private static bool TryLength(JsonElement element, out long? length)
        {
            ...
            foreach (var name in new[] { "filesize", "filesize_approx" })
            {
                ...
                if (value.ValueKind != JsonValueKind.Number || !value.TryGetDouble(out var candidate) ||
                    !double.IsFinite(candidate) || candidate < 0 || candidate >= long.MaxValue) return false;
                length ??= (long)candidate;
            }
