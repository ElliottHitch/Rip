...
public sealed class DownloadApplicationService
{
                            ...
                            else
                            {
                                ...
                                try
                                {
                                    publicationResult = await publisher.PublishAsync(
                                        ...
                                        request.Output with
                                        {
                                            FileStem = SafeFileNamePolicy.Normalize(request.Output.FileStem),
    ...
    private async ValueTask ProgressAsync(RunIdentity run, DownloadStage stage, SequenceCounter sequence)
    {
        ...
        await TerminalAsync(new DownloadProgress(run, stage, sequence.Next(), 0, activity)).ConfigureAwait(false);
    }
    ...
    private static bool IsValidRequest(DownloadRequest? request)
    {
        if (request is null ||
            ...
            !Enum.IsDefined(request.Output.Container))
        {
            return false;
        }
