...
public sealed class ApplicationCompositionTests
{
    ...
    public void Missing_or_mismatched_manifest_is_an_empty_safe_configuration()
    {
        ...
        var manifest = workspace.WriteManifest(executionTargetRid: OperatingSystem.IsWindows() ? "linux-x64" : "win-x64");
    ...
    private sealed class TestWorkspace : IDisposable
    {
        ...
        public string WriteManifest(string? executionTargetRid = null)
        {
            ...
            AddTool("Ffprobe", "ffprobe.sh", repositories[3], "7.0", "#!/bin/sh\nif [ \"$1\" = \"-version\" ]; then printf 'ffprobe version 7.0\\n'; else printf '%s' '{\"format\":{\"duration\":\"1\",\"format_name\":\"mov,mp4\"},\"streams\":[{\"codec_type\":\"video\",\"height\":720,\"width\":1280,\"codec_name\":\"h264\",\"pix_fmt\":\"yuv420p\",\"avg_frame_rate\":\"30/1\",\"bit_rate\":\"500000\"}]}'; fi");
