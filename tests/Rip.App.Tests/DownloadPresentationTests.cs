...
{
    [Fact]
    public void Editing_url_notifies_download_availability_and_quality_survives_mapping()
    {
        var model = ValidVideoModel();
        var changes = new List<string>();
        model.PropertyChanged += (_, args) => changes.Add(args.PropertyName!);
        model.VideoUrl = "https://example.invalid/another";
        model.SelectedQuality = model.VideoQualities.Single(quality => quality.MaximumHeight == 1080);
        Assert.Contains(nameof(model.CanStart), changes);
        Assert.True(model.TryBuildRequest(out var request));
        Assert.Equal(1080, request!.Output.MaximumVideoHeight);
        Assert.False(request.Output.UnifiCompatible);
        model.StartNewRun();
        Assert.Null(model.SelectedQuality.MaximumHeight);
    ...
    public async Task Start_description_reports_safe_state_specific_disabled_reasons()
    {
        ...
        Assert.Contains("Paste a video link", model.StartHelpText, StringComparison.Ordinal);
        model.VideoUrl = "https://example.invalid/video";
        model.OutputFolder = "/safe/destination";
        model.FileStem = "video";
        Assert.Contains("Ready to download", model.StartHelpText, StringComparison.Ordinal);
