...
{
    [Theory]
    [InlineData(null, "4k", 2160)]
    [InlineData(2160, "4k", 2160)]
    [InlineData(1440, "1080", 1080)]
    [InlineData(1080, "1080", 1080)]
    [InlineData(720, "720", 720)]
    [InlineData(480, "progressive", 360)]
    public async Task Realistic_youtube_metadata_keeps_high_quality_and_respects_resolution_ceiling(int? ceiling, string expected, int height)
    {
        var json = """
            {"title":"Example","description":"PADDING","formats":[
              {"format_id":"storyboard","ext":"mhtml","vcodec":"none","acodec":"none"},
              {"format_id":"233","ext":"mp4","vcodec":"none","acodec":null},
              {"format_id":"4k","url":"https://example.invalid/4k","ext":"webm","vcodec":"vp9","acodec":"none","height":2160,"fps":60,"filesize":null,"filesize_approx":1234.5,"preference":-1,"quality":-1},
              {"format_id":"1080","url":"https://example.invalid/1080","ext":"mp4","vcodec":"avc1","acodec":"none","height":1080},
              {"format_id":"720","url":"https://example.invalid/720","ext":"mp4","vcodec":"avc1","acodec":"none","height":720},
              {"format_id":"progressive","url":"https://example.invalid/360","ext":"mp4","vcodec":"avc1","acodec":"mp4a","height":360},
              {"format_id":"audio","url":"https://example.invalid/audio","ext":"webm","vcodec":"none","acodec":"opus"}]}
            """;
        using var tool = FixtureTool.Create(json.Replace("PADDING", new string('x', 70_000), StringComparison.Ordinal));
        var provider = CreateProvider(tool);
        var request = Request(DownloadOperation.Video);
        request = request with { Output = request.Output with { MaximumVideoHeight = ceiling } };
        var metadata = await provider.ReadMetadataAsync(request.Video, null, CancellationToken.None);
        Assert.True(metadata.IsSuccess);
        var result = await provider.ResolveMediaAsync(request, metadata.Value!, CancellationToken.None);
        Assert.True(result.IsSuccess);
        Assert.Equal(expected, result.Value!.VideoFormatId);
        Assert.Equal(height, result.Value.VideoHeight);
        Assert.Equal(expected == "progressive" ? null : "audio", result.Value.AudioFormatId);
    }
    ...
    {
        var formats = string.Join(',', Enumerable.Range(0, 2049).Select(index =>
    ...
    private static BoundedProcessExecutor CreateExecutor(FixtureTool tool) => new(
        ...
        new ProcessExecutorOptions(16 * 1024 * 1024, TimeSpan.FromSeconds(5)),
