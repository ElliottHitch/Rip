...
public sealed class FfmpegProcessAdapterTests
{
    ...
    [Fact]
    public async Task Remux_uses_exact_path_only_non_overwriting_argv_and_returns_opaque_stage_metadata()
    {
        ...
        Assert.Equal(
            ...
            args[..^1]);
        Assert.Equal(Path.GetFullPath(Path.Combine(workspace.StageRoot, result.Value.Artifact.FileName)), args[^1]);
    ...
    public async Task Transcode_uses_frozen_cpu_x264_and_aac_settings()
    {
        ...
        Assert.Equal(
            [
                ...
                "-b:v", "40000000", "-maxrate", "46000000", "-bufsize", "80000000",
                "-profile:v", "high", "-pix_fmt", "yuv420p",
                "-c:a", "aac", "-b:a", "192000", "-ac", "2",
                "-movflags", "+faststart", "-f", "mp4"
            ],
            args[..^1]);
    ...
    [Fact]
    public async Task Equal_known_source_frame_rate_does_not_promote_remux_or_emit_target()
    {
        using var fixture = Fixture.Create(FixtureMode.WriteNonEmptyOutput);
        using var workspace = TestWorkspace.Create();
        var result = await CreateAdapter(fixture).ProcessAsync(
        ...
        Assert.True(result.IsSuccess);
        var args = fixture.ReadArguments();
        Assert.Equal(2, args.Count(static argument => argument == "copy"));
    ...
    public async Task Audio_only_and_video_only_plans_are_mapped_without_inventing_a_second_stream()
    {
        ...
        Assert.Equal(
            [
                ...
                "-map", "0:a:0", "-c:a", "aac", "-b:a", "192000", "-ac", "2",
                "-movflags", "+faststart", "-f", "mp4"
            ],
    ...
    public async Task Missing_trusted_ffmpeg_expectation_is_a_safe_non_launchable_failure()
    {
        ...
        var executor = new BoundedProcessExecutor(
            new Dictionary<string, ToolConfiguration> { [nameof(ToolKey.Ffmpeg)] = fixture.Configuration() },
    ...
    private static BoundedProcessExecutor CreateExecutor(Fixture fixture) => new(
        ...
        "linux-arm64",
        new Dictionary<ToolKey, ToolExpectation> { [ToolKey.Ffmpeg] = fixture.TrustedExpectation() });
